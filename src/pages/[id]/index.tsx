...
const index = () => {
    ...
    return (
        <div>
            ...
            {sessionData ? getUniqueRating.isFetching || getUniqueRating.isLoading ? <div>Loading..</div> : getUniqueRating?.data ? <div>You already Voted</div> : <div className="flex gap-x-5">
                <button onClick={(e:React.FormEvent) => {handle_submit(e,true)}}>Yes</button>
                <button onClick={(e:React.FormEvent) => {handle_submit(e,false)}}>No</button>
            </div>
